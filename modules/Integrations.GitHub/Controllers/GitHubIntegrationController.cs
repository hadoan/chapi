...
public class GitHubIntegrationController : ControllerBase
{
    ...
    [HttpPost("connect")]
    public async Task<ActionResult> Connect(ConnectInstallationRequest req, CancellationToken ct)
    ...
    public async Task<ActionResult> Get(long installationId, CancellationToken ct)
    { var dto = await _service.GetAsync(installationId, ct); return dto == null ? NotFound() : Ok(dto); }
    [HttpPost("checks")]
    public async Task<ActionResult> EnableChecks(EnableChecksRequest req, CancellationToken ct)
    { var dto = await _service.EnableChecksAsync(req, ct); return dto == null ? NotFound() : Ok(dto); }
    [HttpPost("repos")]
    public async Task<ActionResult> AddRepo(AddRepoRequest req, CancellationToken ct)
    { var dto = await _service.AddRepoAsync(req, ct); return dto == null ? NotFound() : Ok(dto); }
    [HttpDelete("repos")]
    public async Task<ActionResult> RemoveRepo([FromBody] RemoveRepoRequest req, CancellationToken ct)
