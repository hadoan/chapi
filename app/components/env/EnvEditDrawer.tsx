...
type Props = {
  ...
  open: boolean;
  onClose: ()=>void;
  onSave: (patch: Partial<EnvModel>)=>void;
}
...
export default function EnvEditDrawer({env, open, onClose, onSave}: Props){
  ...
  function update(patch: Partial<EnvModel>){ setForm(f=> f? {...f, ...patch} : f); }

  function save(){
    // basic validation
    if(!form.baseUrl || !/^https?:\/\//.test(form.baseUrl)) return alert('Enter a valid URL (http/https).');
    if(form.timeoutMs < 1000 || form.timeoutMs >60000) return alert('Timeout must be 1000–60000.');
    onSave(form);
    onClose();
  ...
  async function testConnection(){
    ...
    spinner.textContent = 'Testing...';
    document.body.appendChild(spinner);
    await new Promise(r => setTimeout(r, 800));
    spinner.remove();
    const ok = Math.random() < 0.7;
    alert(ok ? 'Test success (200 OK).' : 'Test failed (timeout).');
  }

  return (
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex">
      <div className="flex-1" onClick={onClose} />
      <div ref={rootRef} className="w-full max-w-md bg-white dark:bg-slate-900 p-4 shadow-lg">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">Edit environment — {form.name}</h2>
        ...
        <div className="space-y-3 mt-3">
          ...
          <div>
            ...
            <KeyValueEditor items={form.headers} onChange={(h)=>update({headers: h})} addLabel="Add header" />
          </div>
          ...
          <div>
            ...
            {Object.entries(form.secrets).map(([k,v])=> (
              <div key={k} className="mb-2">
                ...
                <MaskedInput label={k} value={v} disabled={locked} onChange={(val)=> update({secrets: {...form.secrets, [k]: val}})} />
              </div>
            ))}
          </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button onClick={testConnection} className="px-3 py-1 border rounded">Test Connection</button>
          <button onClick={onClose} className="px-3 py-1 border rounded">Cancel</button>
          <button onClick={save} disabled={locked} className="px-3 py-1 bg-blue-600 text-white rounded">Save</button>
        </div>
