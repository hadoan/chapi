...
export type ApiSpecDto =
  components['schemas']['Chapi.ApiSpecs.Application.ApiSpecDto'];
export type ImportOpenApiRequest =
  components['schemas']['Chapi.ApiSpecs.Application.ImportOpenApiInputDto'];
...
export const apiSpecsApi = {
  async importOpenApi(
    ...
    return await AuthService.authenticatedFetch<ApiSpecDto>(
  ...
  async delete(specId: string): Promise<void> {
    return await AuthService.authenticatedFetch<void>(
      `/api/openapi/${specId}`,
      { method: 'DELETE' }
    );
  },
